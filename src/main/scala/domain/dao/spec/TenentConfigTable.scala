...
trait TenentConfigTable { this: Db =>
  ...
  class TenantConfigs(tag: Tag) extends Table[TenantConfig](tag, Some("eda"), "tenantconfig") {
    ...
    def * : ProvenShape[TenantConfig] = (tenant, domain, host, imapPort, smtpPort, user, passwd, imapSecurity, smtpSecurity, active) <> (TenantConfig.tupled, TenantConfig.unapply)
  }
